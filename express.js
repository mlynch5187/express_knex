...
app.post('/api/v1/footnotes', (request, response) => {
  ...
  for (let requiredParameter of ['paper_id', 'note']) {
    if (!footnote[requiredParameter]) {
      return response
        .status(422)
        .send({ error: `Expected format: { paper_id: <int>, note: <String> }. You're missing a "${requiredParameter}" property.` });
    }
  }
...
// QUERY DATABASE FOR SPECIFIC RESOURCE
...
// RETRIEVE ALL FOOTNOTES OF A PRE-EXISTING PAPER
